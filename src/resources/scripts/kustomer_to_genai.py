...
class CustomerToGenai:
    ...
    def _get_customer_messages(self):
        headers = {
            ...
            "authorization": f"Bearer {self.kustomer_api_key}"
        }
        ...
        parsed_messages = CustomerToGenai._parse_messages(messages)
        sorted_messages = CustomerToGenai._sort_parsed_messages_by_timestamp(parsed_messages)

        for message in sorted_messages:
            self.complete_history += f"{message}"
        print(self.complete_history)
    ...
    def _get_model_output(self):
        ...
        generate_content_config = types.GenerateContentConfig(
            ...
            response_schema=genai.types.Schema(
                ...
                required = ["raciocinio", "nota_final", "criterios", "falha_grave_detectada", "falha_grave_observacao", "observacoes_gerais"],
                properties = {
                    "raciocinio": genai.types.Schema(
                        type = genai.types.Type.STRING,
                    ),
                    "nota_final": genai.types.Schema(
                        type = genai.types.Type.INTEGER,
                    ),
                    "criterios": genai.types.Schema(
                        type = genai.types.Type.OBJECT,
                        required = ["apresentacao_acolhimento", "atencao", "escrita", "respostas_incompletas", "analise_sondagem", "cordialidade_empatia", "uso_respostas_prontas", "avaliacao_tecnica", "registro", "encerramento_despedida", "observacoes_diretrizes_gerais"],
                        properties = {
                            "apresentacao_acolhimento": genai.types.Schema(
                                type = genai.types.Type.INTEGER,
                            ),
                            "atencao": genai.types.Schema(
                                type = genai.types.Type.INTEGER,
                            ),
                            "escrita": genai.types.Schema(
                                type = genai.types.Type.INTEGER,
                            ),
                            "respostas_incompletas": genai.types.Schema(
                                type = genai.types.Type.INTEGER,
                            ),
                            "analise_sondagem": genai.types.Schema(
                                type = genai.types.Type.INTEGER,
                            ),
                            "cordialidade_empatia": genai.types.Schema(
                                type = genai.types.Type.INTEGER,
                            ),
                            "uso_respostas_prontas": genai.types.Schema(
                                type = genai.types.Type.INTEGER,
                            ),
                            "avaliacao_tecnica": genai.types.Schema(
                                type = genai.types.Type.INTEGER,
                            ),
                            "registro": genai.types.Schema(
                                type = genai.types.Type.INTEGER,
                            ),
                            "encerramento_despedida": genai.types.Schema(
                                type = genai.types.Type.INTEGER,
                            ),
                            "observacoes_diretrizes_gerais": genai.types.Schema(
                                type = genai.types.Type.STRING,
                            ),
                        },
                    ),
                    "falha_grave_detectada": genai.types.Schema(
                        type = genai.types.Type.BOOLEAN,
                    ),
                    "falha_grave_observacao": genai.types.Schema(
                        type = genai.types.Type.STRING,
                    ),
                    "observacoes_gerais": genai.types.Schema(
                        type = genai.types.Type.STRING,
                    ),
                },
            ),
            system_instruction=[
                types.Part.from_text(text="""Você é um **Avaliador de Qualidade de Atendimentos** em uma empresa de Delivery, especializado em analisar interações de suporte a clientes que reclamam, exclusivamente, de **pedidos atrasados**.
...
```"""),
        ...
        initial_prompt = f"""Histórico de Mensagens:
```
{self.complete_history}
```
Por favor, primeiramente, faça um **plano de resolução** das etapas para avaliar o atendimento de forma detalhada. **Explique** brevemente sua análise, **critério a critério**, mostrando como você chegaria à nota final. Se necessário, utilize o campo "raciocínio" do output para apresentar essa análise. Elabore, **após isso**, um rascunho da avaliação."""

        chat = client.chats.create(model=model, config=generate_content_config)
        intermediate_response = chat.send_message(initial_prompt)
        print(intermediate_response.text) # Para debug

        follow_up_prompt = f"""Muito obrigado por apresentar o rascunho da avaliação com o plano de resolução claro! Agora, levando em consideração que as diretrizes de avaliação são as mesmas apresentadas anteriormente (pontuações, falhas graves, etc.), sua função é **REVISAR** a avaliação anterior. Se encontrar inconsistências ou falhas, corrija, por favor. Apresente, novamente, seu raciocínio completo."""
    ...
    def main(self):
        self._get_customer_messages()
        response_output = self._get_model_output()
